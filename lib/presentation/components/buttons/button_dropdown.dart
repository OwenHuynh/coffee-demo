...
class _DropdownMenuPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Rect rect = Rect.fromLTRB(
        0, top.evaluate(resize), size.width, bottom.evaluate(resize));
...
class _DropdownMenuItemButtonState<T>
  ...
  Widget build(BuildContext context) {
    ...
    final double start =
        _clampDouble(0.5 + (widget.itemIndex + 1) * unit, 0, 1);
    final double end = _clampDouble(start + 1.5 * unit, 0, 1);
...
class _DropdownMenuRouteLayout<T> extends SingleChildLayoutDelegate {
  ...
  BoxConstraints getConstraintsForChild(BoxConstraints constraints) {
    ...
    double maxHeight = math.max(0, availableHeight - 2 * itemHeight);
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  _MenuLimits getMenuLimits(
    ...
    double computedMaxHeight = availableHeight - 2 * itemHeight;
...
class DropdownButton2<T> extends StatefulWidget {
    ...
    this.formFieldCallBack,
          ...
          items == null ||
              ...
              items.where((DropdownMenuItem<T> item) {
                  ...
                  1,
          "There should be exactly one item with [DropdownButtonFormField] "
          '$value. \n'
          'Either zero or 2 or more [DropdownMenuItem]s were detected '
          'with the same value',
        );
  ...
  /// Uses different predefined heights for the menu items
  /// (useful for adding dividers)
  final List<double>? customItemsHeights;
  ...
  /// if it is non-null.
  ///
  /// If [value] is null and the button is disabled, [disabledHint]
  /// will be displayed
  /// if it is non-null. If [disabledHint] is null, then [hint]
  /// will be displayed
  /// if it is non-null.
  final T? value;
  ...
  ///
  /// If [value] is null and the dropdown is enabled
  /// ([items] and [onChanged] are non-null),
  ...
  /// A preferred placeholder widget that is
  /// displayed when the dropdown is disabled.
  ///
  /// If [value] is null, the dropdown is disabled
  /// ([items] or [onChanged] is null),
  /// this widget is displayed as a placeholder for the dropdown button's value.
  ...
  /// If [DropdownButton2.disabledHint] is also null but [DropdownButton2.hint]
  /// is non-null, [DropdownButton2.hint] will instead be displayed.
  /// {@endtemplate}
  ...
  ///
  /// ** See code in examples/api/lib/material/dropdown/dropdown_button.selected_item_builder.dart **
  /// {@end-tool}
  ///
  ...
  ///
  /// ** See code in examples/api/lib/material/dropdown/dropdown_button.style.dart **
  /// {@end-tool}
  ///
  ...
  ///
  /// Defaults to a 0 width bottom border with color 0xFFBDBDBD.
  ...
  /// It defaults to [AlignmentDirectional.centerStart].
  ///
  /// See also:
  ///
...
class DropdownButton2State<T> extends State<DropdownButton2<T>>
  ...
  // Using ValueNotifier for the Rect of DropdownButton
  //so the dropdown menu listen and
  // update its position if DropdownButton's position has changed,
  //as when keyboard open.
  ...
  void didChangeMetrics() {
    ...
    if (_rect.value!.top == _newRect.top) {
      return;
    }
  ...
  double _getMenuHorizontalPadding() {
    final menuHorizontalPadding =
        (widget.itemPadding?.horizontal ?? _kMenuItemPadding.horizontal) +
            (widget.dropdownPadding?.horizontal ?? 0) +
            (widget.dropdownScrollPadding?.horizontal ?? 0);
      ...
      for (int index = 0; index < widget.items!.length; index += 1)
        _MenuItem<T>(
          ...
          onLayout: (Size size) {
            ...
            // of each menu item before laying them out, not having the
            // _DropdownRoute
  ...
  // This expose the _handleTap() to Allow opening the button programmatically
  // using GlobalKey.
  // Also, DropdownButton2State should be public as we need typed access to it
  // through key.
  void callTap() => _handleTap();

  // When isDense is true,
  //reduce the height of this button from _kMenuItemHeight to
  // _kDenseButtonHeight,
  //but don't make it smaller than the text that it contains.
  ...
  Widget build(BuildContext context) {
    ...
    if (items.isEmpty) {
      ...
      innerItemsWidget = Padding(
        //When buttonWidth & dropdownWidth is null,
        //their width will be calculated
        //from the maximum width of menu items or the hint text
        //(width of IndexedStack).
        //We need to add MenuHorizontalPadding so menu width adapts
        //to max items width with padding properly
    ...
    if (!DropdownButtonHideUnderline.at(context)) {
      final double bottom = widget.isDense ? 0 : 8;
      result = Stack(
        children: <Widget>[
          result,
          Positioned(
...
class DropdownButtonFormField2<T> extends FormField<T> {
  ...
  /// It is useful for some cases, i.e: calling callTap()
  /// method to open the menu programmatically
